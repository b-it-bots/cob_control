...
void CobTwistController::checkSolverAndConstraints(cob_twist_controller::TwistControllerConfig& config)
{
    ...
    if (DEFAULT_SOLVER == solver && (JLA_OFF != static_cast<ConstraintTypesJLA>(config.constraint_jla) ||
    ...
    if (GPM == solver && CA_OFF == static_cast<ConstraintTypesCA>(config.constraint_ca) &&
        JLA_OFF == static_cast<ConstraintTypesJLA>(config.constraint_jla))
    {
    ...
    if (TASK_2ND_PRIO == solver && (JLA_ON == static_cast<ConstraintTypesJLA>(config.constraint_jla) ||
        CA_OFF == static_cast<ConstraintTypesCA>(config.constraint_ca)))
    {
